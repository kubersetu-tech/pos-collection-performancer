...
const GoogleSheetPieWithTable = () => {
  ...
  const processData = (rawData) => {
    ...
    const pieData = rows
      .filter((row) => 
        SHOW_ONLY_NAMES.includes(String(row[0]).toLowerCase().trim())
      )
      .map((row) => {
        // Extract percentage value
        let value = 0;
        for (let i = 1; i < Math.min(row.length, 3); i++) {
    ...
    const averagePerformance = pieData.reduce((sum, item) => sum + item.value, 0) / pieData.length;
  ...
  if (loading && data.length === 0) return (
    <div className="loading-container">
      ...
      <p>Loading PaisaOnSalary dashboard...</p>
      <p className="loading-subtext">Fetching real-time data from Google Sheets</p>
    </div>
  );
    ...
    <div className="dashboard-container">
      ...
      <div className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-menu">
          ...
          <button 
            className={`sidebar-item ${activeView === 'charts' ? 'active' : ''}`}
        ...
        <div className="sidebar-footer">
          <div className="stats-summary">
            ...
            <div className="summary-item">
              ...
              <span className="summary-value">{pieData.length}</span>
            </div>
            <div className="summary-item">
              <span>Avg Performance</span>
              <span className="summary-value">
                {pieData.length > 0 ? 
                  (pieData.reduce((sum, item) => sum + item.value, 0) / pieData.length).toFixed(1) : 0}%
              </span>
            </div>
            <div className="summary-item">
              <span>Top Performer</span>
              <span className="summary-value">
                {topPerformers[0]?.name || 'N/A'} ({topPerformers[0]?.value || 0}%)
              </span>
            </div>
          </div>
        </div>
      ...
      <main className="main-content">
        {/* Header Stats Cards - Navbar का data अब यहाँ दिखेगा */}
            ...
            {pieData.map((employee, index) => {
              ...
              return (
                <div className="employee-card" key={index}>
                  {showAlert && (
                    <div className={`employee-alert ${employee.value >= 80 ? 'good' : 'bad'}`}>
                      {employee.value >= 80 ? 
                        <CheckCircle2 size={14} /> : 
                        <AlertTriangle size={14} />
                      }
                      <span>
                        {employee.value >= 80 ? 'Excellent Performance' : 'Needs Improvement'}
                      </span>
                    </div>
                  )}
        ...
        <div className="table-card">
          <div className="table-header">
            <div>
              ...
              <p>Detailed metrics for all employees • Last updated: {formatTime(lastUpdated)}</p>
            </div>
      ...
      <style jsx>{`
        ...
        * {
        ...
        /* Header Stats Section - Navbar का data अब यहाँ */
